...
namespace Api_PPS
{
    ...
    const int TamanioPagina = 5;
        ...
        {
            logger.LogInformation("Obteniendo todos los videojuegos con filtros - G�nero: {Genero}, Nombre: {Nombre}", genero, nombre);

            var (videojuegos, totalRegistros) = await gestionVideojuegos.ObtenerTodosFiltrado(genero, nombre, numeroPagina, tamanioPagina);

            logger.LogInformation("Obteniendo todos los videojuegos. N�mero de videojuegos: {TotalRegistros}", totalRegistros);
        ...
        public async Task<ActionResult<Videojuego>> Post([FromBody] Videojuego videojuego)
        {
            ...
            if (!modeloValido)
            {
                logger.LogWarning("Modelo de videojuego no v�lido para: {Nombre}", videojuego.Nombre);
        ...
        public async Task<ActionResult> Put([FromRoute] int id, [FromBody] Videojuego videojuego)
        {
            ...
            {
                logger.LogError("No se encontr� el videojuego con id {Id} para actualizar", id);
                return NotFound();
            }
        ...
        public async Task<ActionResult> Delete([FromRoute] int id)
        {
            ...
            if (videojuego == null)
            {
                logger.LogError("No se encontr� el videojuego con id {Id} para eliminar", id);
                return NotFound();
            }
